...
data <- rbind(data_neuro, data_wesad, data_ubfc, data_exam)
data <- data %>% select(hr, eda, Subject, metric) # 115

rm(data_neuro, data_wesad, data_ubfc, data_exam)
gc()
...
make_swell_data <- function(folder, feature_engineering = FALSE)
{
  ...
  {
    swell <- read.csv(paste(folder, '/p', subject, '.csv', sep=''))
    hr <- swell$hr
    eda <- swell$eda
    metric <- swell$metric - 1
...
data_swell <-make_swell_data('SWELL', feature_engineering = TRUE)
...
#########################################################################################################################################################
# Synthesize 200 subjects, train our synthetic stress dataset, test on SWELL, 10 times
#########################################################################################################################################################
main_results <- NULL
...
write.csv(main_results, "Supplement10.csv", row.names = FALSE)
print(mean(main_results$XGB)) # 0.5493266
print(mean(main_results$ANN)) #  0.6171973
print(mean(main_results$Ensemble)) # 0.5826176

print(sd(main_results$XGB)) # 0.07984168
print(sd(main_results$ANN)) # 0.06051015
print(sd(main_results$Ensemble)) # 0.07162345
