...
for (subject in subjects)
{
  ...
  write.csv(results, "ResultsWesadPredict.csv",row.names = FALSE)
}
...
print(mean(results$XGB, na.rm=TRUE)) # 0.80
print(mean(results$ANN, na.rm=TRUE)) # 0.77
print(mean(results$ENS, na.rm=TRUE)) # 0.79
print(mean(results$PRECISION, na.rm=TRUE)) # 0.74
