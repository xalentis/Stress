...
data <- data %>% select(hrmax, hrmin, hrstd, hrmedian, hr, edaskew, edastd, hrmean, eda, edarange, Subject, Balance, metric)
...
# 360/360 [==============================] - 1s 2ms/step - loss: 0.1417 - val_loss: 0.1411
...
exam_data <- exam_data %>% select(hrmax, hrmin, hrstd, hrmedian, hr, edaskew, edastd, hrmean, eda, edarange, Subject)
...
ggplot(temp, aes(x=ID)) + 
  geom_line(aes(y = xgb, colour="XGB"),  alpha=0.4, size=0.5) + 
  geom_line(aes(y = ann, colour="ANN"),  alpha=0.4, size=0.5) + 
